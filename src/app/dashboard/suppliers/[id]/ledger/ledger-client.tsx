...
export default function SupplierLedgerClient({ supplierId }: { supplierId: string }) {
  ...
  useEffect(() => { fetchData(); }, [supplierId, dateRange]);
  ...
  return (
    <div className="space-y-6 p-6 print:p-0">
      <div className="flex items-center gap-4 mb-4 print:hidden">
        <Link href="/dashboard/suppliers"><Button variant="outline" size="icon"><ArrowLeft className="h-4 w-4" /></Button></Link>
        <span className="text-sm text-muted-foreground">Back to Suppliers</span>
      </div>
        ...
        <div>
          ...
          {data && (
            <p className="text-muted-foreground mt-1">
              Account statement for <span className="font-semibold text-foreground">{data.supplier.name}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:grid-cols-3">
            <Card className="bg-blue-50/50 dark:bg-blue-950/20 border-blue-100 dark:border-blue-900">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{formatCurrency(data.summary.totalPurchased)}</p>
              </CardContent>
            </Card>
            <Card className="bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900">
              <CardContent className="p-4">
                <p className="text-xs text-muted-foreground">Total Paid</p>
                <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(data.summary.totalPaid)}</p>
              </CardContent>
            </Card>
            <Card className="bg-red-50/50 dark:bg-red-950/20 border-red-100 dark:border-red-900">
              <CardContent className="p-4">
                <p className="text-xs text-muted-foreground">Current Balance Payable</p>
                <p className="text-2xl font-bold text-red-600 dark:text-red-400">{formatCurrency(data.summary.balanceDue)}</p>
              </CardContent>
            </Card>
          </div>
